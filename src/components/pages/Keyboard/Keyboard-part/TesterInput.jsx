...
import { changeInputValue, showFocusedSign } from "./function/TesterInput.js";
...
export const TesterInput = ({ onKeyDown }) => {
  ...
  const handleClick = () => {
    setFocused(true);
    if (focused) {
      const inputPreview = document.querySelector(".keyboard__tester-input__preview");
  ...
  return (
    ...
    >
      <div
        className="keyboard__tester-input__word fs-1 px-4 py-1 text-light text-nowrap px-2"
        style={{ transform: `translateX(-${translationX}%)` }}
      >
        {loading ? <Loading /> : word}
      </div>
      <div
        className="keyboard__tester-input__answer fs-1 px-4 py-1 text-light text-nowrap"
        style={{ transform: `translateX(-${translationX}%)` }}
      >
        {lastInput}
      </div>
